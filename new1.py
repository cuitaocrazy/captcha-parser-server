...
def parse(example_proto):
  ...
  image = tf.gather(tf.image.decode_png(single['img_raw']), axis=2, indices=[0, 1, 2])
  return image / 255, tf.one_hot(single['label'], 10)

dataset = data.map(parse).shuffle(buffer_size = 6400).batch(640)

model = tf.keras.models.Sequential()
model.add(tf.keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=(28, 72, 3)))
